...
(function() {
    ...
    // Our display boxes
    var speedBox = geid("speed");
    var timeBox = geid("totalTime");
    var media = geid("media");
    var marksBox = geid("marks");
    var jumpBox = geid("jump");
    var jumpToBox = geid("jumpTo");
    var curPosBox = null;
    ...
    var curPos = 0;
    var curPrevMark = null, curIndex = 0, curNextMark = null;
    var totalTime = 0;
    ...
    var waveform = null, waveformDiv = 1, wavesurferOffset = -1, wavesurferDuration = 600;
    var wavesurferZoom = 64;
    (function() {
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "/wave.json", true);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                waveform = JSON.parse(xhr.responseText);
                waveformDiv = 1 << (waveform.bits-1);
                updateWavesurfer();
            }
        }
        xhr.send();
    ...
    (function() {
        ...
        xhr.open("GET", "/in.mark", true);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                var marksRaw = xhr.responseText.split("\n");
                marksRaw.forEach(function (marksLine) {
                    if (marksLine.length)
                        marks.push({"e": marksLine[0], "t": Number(marksLine.slice(1))});
                });
            }
            curPrevMark = null;
            curNextMark = marks[0]?marks[0]:null;
            updateMarks();
        }
    ...
    var marksSending = false;
    var sendAgain = false;

    // Send marks to the server
    function sendMarks() {
        if (marksSending) {
            // Send them again when it's done
            sendAgain = true;
            return;
        }
